...
{ 
   HANDLE h= GetStdHandle(STD_OUTPUT_HANDLE);
   BUSMANGEMENT obj; 
   int addedbuses=0; // counter for bus added
   // creating the object
	int totalbuses=3;
	...
	SetConsoleTextAttribute(h,8);
	cout<<"\nEnter the number of buses that are is to be add: ";
	cin>>busrequire;
	...
	switch(choice)
{
	case 1:
		// adding buses
		if(addedbuses<busrequire)
		{ 
			obj.add(obj.a);
			addedbuses++;
			cout<<"BUS "<<addedbuses<<"added successfully.\n";
			if(addedbuses==busrequire)
			{
				cout<<"all buses are added";
			}
		}
		else
		{
			cout<<"Error you have already added all buses";
		}
	   break;
	case 2:
		if(addedbuses<busrequire)
		{
			cout<<"Error: You must add all buses first before deleting.\n";
		}
		else
		{
			obj.del(obj.a);	
	    ...
	    case 2:
		   ...
		   case 3:
		    ...
		    if(distance1>distance2&&distance1>distance3)
		    {
           	...
           	SetConsoleTextAttribute(h,2);
		    	cout<<"\n As the distance of bus1 is more the time taken by bus1 is more "<<endl;
			}
			else if(distance2>distance1&&distance2>distance3)
			{
				
           	SetConsoleTextAttribute(h,1);
		    	cout<<"As the distance of bus2 is more so the time taken by bus2 is more"<<endl;
			}
			else
			{
			
           	SetConsoleTextAttribute(h,5);
				cout<<"As the distance of bus 3 is more so the time taken by bus3 is more"<<endl;
